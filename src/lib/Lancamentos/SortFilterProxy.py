...
from PyQt5.QtCore import ( 
    QModelIndex, QObject, QSortFilterProxyModel, QRegExp, Qt
)
import moment.utils
...
class FilterCategoria():
    ...
    def validate(self, categoria: str) -> bool:
        regex = QRegExp(
            self.categoria, Qt.CaseInsensitive, QRegExp.RegExp
        )
...
class LancamentoSortFilterProxyModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row: int, source_parent: QModelIndex) -> bool:
        ...
        for single_filter in [x for x in self.filters]:
            ...
            if not isinstance(filter_categoria, FilterCategoria) or \
                not isinstance(filter_ano_mes, FilterAnoMes):
                continue

            # if not isinstance(filter_ano_mes, FilterAnoMes):
            #     return True
